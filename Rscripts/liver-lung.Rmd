...
liver_peaks = my_read_bed("results/ST001-liver/hap1-vs-hap2/FIRE.hap.differences.bed")
liver_track = my_read_bed("../FIREv2.0/results/ST001-liver/FDR-peaks/FDR.track.bed.gz")
lung_hap_peaks = lung_peaks %>% filter(p_adjust <= 0.05)
```
...
```{r}
min_hap_cov = 10
filtered_tissue_peaks = tissue_peaks %>%
...
sum(is.na(filtered_tissue_peaks$diff))

tissue_peaks_compare_df = filtered_tissue_peaks %>%
    bed_map(
        lung_hap_peaks,
        lung_diff = diff[which.min(abs(diff))],
        nn = n(),
    ) %>%
    filter(!is.na(lung_diff)) %>%
    bed_map(
        imprinted, 
        is_imprinted = n()>0,
    ) %>%
...
my_ggsave("Figures/lung-liver-hap-peaks.pdf", height=4, width=4)
```
