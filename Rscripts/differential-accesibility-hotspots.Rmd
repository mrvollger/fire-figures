...
my_ggsave("Figures/inter-sample-epi-comparison-rank-ideogram.pdf", width=12, height=22)
```
...
merged_both_rank2 %>% 
    fwrite("Figures/inter-sample-epi-comparison-both-rank-two.txt", sep="\t", quote=F, na="NA")
...
zz
zz %>%
    fwrite("Figures/inter-sample-epi-comparison-SD-alt-overlaps.txt", sep="\t")


my_ggsave("Figures/inter-sample-epi-comparison-SD-alt-overlaps.pdf", width=4, height=3)
...
rank_100k %>%
    ...
    theme(legend.position="top")
my_ggsave("Figures/inter-sample-epi-comparison-rank-histogram.pdf", height=6, width=8)
```
...
my_ggsave("Figures/significant-enrichment-of-hap-specific-peaks.pdf", width=8, height=3)
```

```{r}
hla_classes = my_read_bed("chr6 32430523 33480577 HLA-class-II
chr6 31564074 32430523 HLA-class-III
chr6 29694717 31564074 HLA-class-I
", sep=" ") %>%
    mutate(width = end - start) %>%
    expand_grid(my_group=unique(to_use$my_group)) %>%
    group_by(my_group) %>%
    bed_map(
        to_use %>%
            group_by(my_group) %>%
        ...
        #window_start = ostart[which.min(p_adjust)],
        #window_end = oend[which.min(p_adjust)],
        midpoint = ostart[which.min(p_adjust)],
        n_hap_peaks = n_hap_peaks[which.min(p_adjust)],
        n_not_hap_peaks = n_not_hap_peaks[which.min(p_adjust)],
        tn_hap_peaks = tn_hap_peaks[which.min(p_adjust)],
        tn_all_peaks = tn_all_peaks[which.min(p_adjust)],
        p_value = min(p_value),
        p_adjust = min(p_adjust),
    ) %>%
    data.table(); colnames(hla_classes)[4] = "class"
hla_classes %>% filter("CD8+"==my_group)
hla_classes
...
rr=to_use %>%
    ...
    filter(!is.na(hla_class))  %>%
    mutate(
        class=hla_class
    ) %>%
    group_by(
        my_group, class, p_value, p_adjust,
        n_hap_peaks, n_not_hap_peaks, tn_hap_peaks, tn_all_peaks,
    ) %>%
    summarise(
        start=min(start),
        end=max(end),
...
my_ggsave("Figures/significant-enrichment-of-hap-specific-peaks-HLA-classes.pdf", width=3, height=2)
