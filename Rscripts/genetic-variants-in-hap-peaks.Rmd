...
pbsv_ends = pbsv_vcf %>%
    ...
    filter(!is.na(end))
    #%>%unnest()
max(pbsv_vcf$end-pbsv_vcf$start, na.rm=T)
```
...
bw=5
bind_rows(list(hap_peaks_with_null %>% filter(type=="real"), not_hap_sel_df)) %>%
    mutate(
    ...
    geom_text(
        data = . %>%
            ...
            dplyr::summarise(count = n(), stat = mean(diff_5mC)),
...
my_ggsave("Figures/hap-selective-5mC-difference-in-peaks.pdf", height=3, width=4)
```
