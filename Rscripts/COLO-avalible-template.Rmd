...
fire = fread("results/PS00357_COLO829T_1/FDR-peaks/FDR-FIRE-peaks.bed.gz") %>%
    ...
    mutate(
        o_coverage_H1 = coverage_H1,
        o_coverage_H2 = coverage_H2,
        o_fire_coverage_H1 = fire_coverage_H1,
        o_fire_coverage_H2 = fire_coverage_H2,
    )
...
median(fire$coverage_H1)
median(fire$coverage_H2)
...
fire %>%
    pivot_longer(
        cols = c("o_coverage_H1", "o_coverage_H2"),
        names_to = "type",
        values_to = "coverage_hap"
    ) %>%
    ggplot(aes(x=coverage_hap, fill=type)) +
    geom_density(alpha=0.75, adjust=0.6) +
    scale_fill_manual("",
        labels=c("Haplotype 1", "Haplotype 2"),
        values=c("skyblue", "darkred"),
    ) +
    scale_x_continuous("Coverage", limits=c(0,100), breaks=seq(0,1000,25)) +
    my_grid()+
    theme(legend.position="top")
my_ggsave("Figures/COLO-T-coverage.pdf", height=3, width=3) 
```
