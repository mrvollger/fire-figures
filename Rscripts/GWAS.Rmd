...
long.gwas.validation %>%
    ...
    scale_fill_manual("", 
        values=c(
            "FIRE peak without ATAC-seq"="red",
            "FIRE peak with ATAC-seq"="darkred",
...
z=wide.with.shuffle.gwas.validation %>%
    ...
    geom_text(
        aes(
            label=paste0(round(5, 4), "\nn=", peaks_with_all_gwas_FALSE)
        ),
        position=position_dodge(width=0.6),
        vjust=0.5,
        size=1, 
    ...
    guides(fill="none")+
    my_grid()
my_ggsave("Figures/gwas-per-kbp-of-fire-peak-enrichment.pdf", height=2, width=4)
