...
z= fire_df %>%
    ...
    filter(!is.na(MAF)) %>%
    select(chrom,start,end,MAF, hap_selective)
...
dnase_peaks
sr_only_peaks = bind_rows(
        list(
            atac=atac_peaks %>% mutate(is_atac_peak=T), 
            dnase=dnase_peaks %>% mutate(is_dnase_peak=T)
        )
    ) %>%
    bed_map(
        fire_df,
...
gwas.validation = bind_rows(
    list(
        ...
        atac = sr_only_peaks %>% filter(is_atac_peak==TRUE),
        dnase = sr_only_peaks %>% filter(is_dnase_peak==TRUE) 
    ),
    .id="shuffled"
    ) %>%
    bed_map(
...
long.gwas.validation = gwas.validation %>%
    mutate(
        case = case_when(
            shuffled =="shuffled" ~ "Random windows",
            shuffled =="atac" ~ "ATAC-seq only peaks",
            shuffled =="dnase" ~ "DNase-seq only peaks",
            is_atac_peak ~ "FIREs that overlap ATAC/DNase",
            TRUE ~ "FIRE only peaks"
        ),
        is_shuffled = shuffled=="shuffled",
    ) %>%
    pivot_longer(
...
wide_cols = c("case", "n_peaks", "peaks_with_gwas", "gwas_in_peaks",
        "peaks_with_all_gwas", "all_gwas_in_peaks", "gwas_per_kbp_of_fire_peak", "peaks_without_gwas")
after_wide_cols = paste0(wide_cols, "_","FALSE")
after_wide_cols_2 = paste0(wide_cols, "_","TRUE")
wide.with.shuffle.gwas.validation = long.gwas.validation %>%
    filter(gwas_type != "all_gwas_per_kbp_of_fire_peak") %>%
    mutate(peaks_without_gwas = n_peaks-gwas_in_peaks) %>%
    ...
    rowwise() %>%
    filter(n_peaks_TRUE > 0) %>%
    mutate(
        test = (fisher.test(matrix(c(
            peaks_with_gwas_TRUE,
            peaks_without_gwas_TRUE,
            peaks_with_gwas_FALSE,
            peaks_without_gwas_FALSE
        ), nrow=2))$p.value),
    ) %>%
    ungroup %>% 
    data.table
...
long.gwas.validation %>%
    ggplot(aes(y=case, x=gwas_per_kbp_of_fire_peak, fill=case)) +
    geom_bar(stat="identity", position="dodge", width=0.6) +
    geom_text(
        aes(
            label=paste0(round(gwas_per_kbp_of_fire_peak, 4), "\nn=", peaks_with_all_gwas)
        ),
    ...
    scale_fill_manual("", 
        values=c(
            ...
            "DNase-seq only peaks"="darkblue",
            "ATAC-seq only peaks"="darkblue",
...
z=wide.with.shuffle.gwas.validation %>%
    ggplot(
        aes(y=case_FALSE,
            x=gwas_per_kbp_of_fire_peak_FALSE/gwas_per_kbp_of_fire_peak_TRUE, 
            fill=case_FALSE
            )
    ) + 
    geom_bar(stat="identity", position="dodge", width=0.6) +
    geom_text(
        aes(
            label=paste0( "n=", peaks_with_all_gwas_FALSE)
        ),
        position=position_dodge(width=0.6),
        vjust=0.5,
        size=1, 
    ...
    scale_x_continuous(
        "Enrichment of GWAS variants in peaks compared to random windows",
    )+
    ylab("")+
    scale_fill_manual("", 
        values=c(
